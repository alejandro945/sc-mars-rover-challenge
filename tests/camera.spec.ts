...
test('test', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: 'Rover' }).selectOption('opportunity');
  await page.getByRole('combobox', { name: 'Date Filter' }).selectOption('earth_date');
  await page.getByLabel('Earth date').fill('2015-06-03');
  await page.getByRole('combobox', { name: 'Camera' }).selectOption('PANCAM');
  await page.getByRole('button', { name: 'Filter' }).click();
  await page.locator('.chakra-stack > .chakra-heading').first().click();
});